...
';
	...
	public static function write($data, $filename, $namespace) {
		$file = fopen(base_path(static ::$full_path.'/'.str_replace('\\', '/', $namespace)).'/'.$filename.'.php', "w");
		fwrite($file, $data);
		fclose($file);
	}
...
<div class="row">
	<div class="col-md-12">
		...
		if ($r->has('col_type') and $r->has('col_name')) {
			...
			foreach ($r->input('col_name') as $col_name) {
				...
				if (!empty($r->input('col_type')[$i]) and $r->input('col_type')[$i] == 'text') {
					$input .= BaboonCreate::text($data);
				} elseif (!empty($r->input('col_type')[$i]) and $r->input('col_type')[$i] == 'email') {
					$input .= BaboonCreate::email($data);
				} elseif (!empty($r->input('col_type')[$i]) and $r->input('col_type')[$i] == 'select') {
					$input .= BaboonCreate::select($data);
				} elseif (!empty($r->input('col_type')[$i]) and $r->input('col_type')[$i] == 'url') {
					$input .= BaboonCreate::url($data);
				} elseif (!empty($r->input('col_type')[$i]) and $r->input('col_type')[$i] == 'password') {
					$input .= BaboonCreate::password($data);
				} elseif (!empty($r->input('col_type')[$i]) and $r->input('col_type')[$i] == 'textarea') {
					$input .= BaboonCreate::textarea($data);
				} elseif (!empty($r->input('col_type')[$i]) and $r->input('col_type')[$i] == 'date') {
					$input .= BaboonCreate::date($data);
				} elseif (!empty($r->input('col_type')[$i]) and $r->input('col_type')[$i] == 'checkbox') {
					$input .= BaboonCreate::checkbox($data);
				} elseif (!empty($r->input('col_type')[$i]) and $r->input('col_type')[$i] == 'radio') {
					$input .= BaboonCreate::radio($data);
				} elseif (!empty($r->input('col_type')[$i]) and $r->input('col_type')[$i] == 'file') {
					$input .= BaboonCreate::file($data);
				} elseif (!empty($r->input('col_type')[$i]) and $r->input('col_type')[$i] == 'color') {
					$input .= BaboonCreate::color($data);
				} elseif (!empty($r->input('col_type')[$i]) and $r->input('col_type')[$i] == 'time') {
	...
	public static function Makelang($r, $folder = 'ar') {
		...
		$the_master_lang += ['deleted' => 'تم الحذف بنجاح'];*/
