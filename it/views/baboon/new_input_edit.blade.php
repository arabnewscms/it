      ...
      <div id="rulesCard{{ $x }}">
        <div class="card">
          <div class="card-header">
            <a class="card-link" data-toggle="collapse" data-parent="#rulesCard{{ $x }}" href="#basic_rules{{ $x }}">Basic Rules (<span class="col_name_{{ $x }}">{{ $input->col_name }}</span>)</a>
          </div>
        ...
        <div class="card">
          <div class="card-header">
            <a class="collapsed card-link" data-toggle="collapse" data-parent="#rulesCard{{ $x }}" href="#advanced_rules{{ $x }}">Advanced Rules (<span class="col_name_{{ $x }}">{{ $input->col_name }}</span>)</a>
          </div>
          ...
          <div class="form-group col-md-4">
            ...
            <div class="col-md-4">
              ...
              @endif
                ...
                <div class="clearfix"></div>
                <p>$table->bigInteger('<span class="col_name_{{ $x }}">{{explode('|',$input->col_name_convention)[0]}}</span>')->unsigned()<span class="func_nullable{{ $x }} {{ !empty(!empty($rules->{'forginkeyto'.$x})) && !empty($rules->{'forginkeyto'.$x}[1]) && !empty($rules->{'forginkeyto'.$x}[1]->{'schema_onDelete'.$x})?'':'hidden' }}">->nullable()</span>;</p>
                <p>$table->foreign('<span class="col_name_{{ $x }}">{{explode('|',$input->col_name_convention)[0]}}</span>')->references('<span class="references{{ $x }}">@if(!empty(!empty($rules->{'forginkeyto'.$x})) && !empty($rules->{'forginkeyto'.$x}[1]) && !empty($rules->{'forginkeyto'.$x}[1]->{'references'.$x})){{ $rules->{'forginkeyto'.$x}[1]->{'references'.$x} }}@endif</span>')->on('<span class="forgin_table_name{{ $x }}">@if(!empty(!empty($rules->{'forginkeyto'.$x})) && !empty($rules->{'forginkeyto'.$x}[1]) && !empty($rules->{'forginkeyto'.$x}[1]->{'forgin_table_name'.$x})){{ $rules->{'forginkeyto'.$x}[1]->{'forgin_table_name'.$x} }}@endif</span>')<span  class="schema_onDelete{{ $x }} {{ !empty(!empty($rules->{'forginkeyto'.$x})) && !empty($rules->{'forginkeyto'.$x}[1]) && $rules->{'forginkeyto'.$x}[1]->{'schema_onDelete'.$x}?'':'hidden' }}">->onDelete('cascade')</span><span  class="schema_onUpdate{{ $x }} {{ !empty(!empty($rules->{'forginkeyto'.$x})) && !empty($rules->{'forginkeyto'.$x}[1]) && $rules->{'forginkeyto'.$x}[1]->{'schema_onUpdate'.$x}?'':'hidden' }}">->onUpdate('cascade')</span>;</p>
              </div>
            </div>
            <div class="clearfix"></div>
          </div>
